...
module Top_Student(input basys_clock, input btnC, inout PS2Clk, inout PS2Data, output [7:0] JB, output [15:0] led);
    ...
    parabolic_motion unit_1(.basys_clock(basys_clock), .collision_flag(collision_flag), .clk_15Hz(clk_15Hz), .btnC(btnC), .x_coordinate(x_coordinate), 
    .y_coordinate(y_coordinate), .oled_data(oled_fruit));
    ...
    always @(posedge clk_25MHz) begin
        if ((oled_mouse == 16'hF800) && oled_fruit == 16'b11111_111000_00000) begin
            collision_flag <= (count == 30) ? 1 : 0;
            count <= count + 1; 
        end
        if (oled_mouse == 16'hF800 || oled_mouse == 16'h0000) begin
            oled_colour <= oled_mouse; 
        end else if (oled_fruit == 16'b11111_111000_00000) begin
            oled_colour <= oled_fruit;
        end else begin
